...
class JavaScriptCommentExtractor(CommentExtractor):
    def __init__(self):
        ...
        # 字符串规则
        self.__string_pattern1 = r'\"([\s\S]*?)\"'
        self.__string_pattern2 = r'\'([\s\S]*?)\''
        self.__string_pattern3 = r'\`([\s\S]*?)\`'
    ...
    def extract_comments(self, code):
        ...
        if self.__string_pattern1 is not None:
            pattern += '|'
            pattern += self.__string_pattern1
        if self.__string_pattern2 is not None:
            pattern += '|'
            pattern += self.__string_pattern2
        if self.__string_pattern3 is not None:
            pattern += '|'
            pattern += self.__string_pattern3
        ...
        for match in matches:
            single_line, mult_line, str_line1, str_line2, str_line3 = match.groups()
            if single_line:
